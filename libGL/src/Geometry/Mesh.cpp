...
Mesh::~Mesh()
{
	if( m_pVertexInfo )
	{
		GLVBOResourceManager::GetInstance().HandleDelete( m_pVertexInfo->GetVertexBufferLoc() );

		if( m_pVertexInfo->GetUseIndices() )
			GLIBOResourceManager::GetInstance().HandleDelete( m_pVertexInfo->GetIndexBufferLoc() );
	}
...
{
	GLVBOResourceManager::GetInstance().AddResource( pNewVertexInfo->GetVertexBufferLoc() );
	
	if( pNewVertexInfo->GetUseIndices() )
		GLIBOResourceManager::GetInstance().AddResource( pNewVertexInfo->GetIndexBufferLoc() );
