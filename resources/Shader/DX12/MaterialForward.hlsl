...
//---------------------------------------------------------------------------//
    ...
    VS_OUT main(VS_IN v)
    {
      ...
      vs_out.pos = mul(float4(v.position, 1.0f), c_WorldViewProjectionMatrix);

      vs_out.posWS = mul(float4(v.position, 1.0f), c_WorldMatrix).xyz;
      vs_out.normalWS = normalize(mul(float4(v.normal, 0.0), c_WorldMatrix).xyz);
