...
std::vector<glm::vec3> Camera::getWSfrustumCorners()
{
	...
	float aspect = (float) m_pEngine->GetScreenWidth() / (float) m_pEngine->GetScreenHeight();
...
void Camera::CheckOriantationMatrix( const uint elapsedTicksMS )
{
	...
	float angleX = currentMState->m_relativeMouseMovement.x * m_pEngine->GetMovementMul();
	float angleY = currentMState->m_relativeMouseMovement.y * m_pEngine->GetMovementMul();

	angleX += currentMState->m_relativeMouseMovement.x * m_pEngine->GetMovementMul();
	angleY += currentMState->m_relativeMouseMovement.y * m_pEngine->GetMovementMul();
...
{
	m_v3Position += m_v3View * fSpeed * m_pEngine->GetMovementMul();
}

void Camera::MoveSideways( float fSpeed )
{
	m_v3Position += m_v3Side * fSpeed * m_pEngine->GetMovementMul();
}