...
namespace Fancy {
  namespace ImGuiRendering {
    ...
    bool Init( RenderOutputHandle aRenderOutput ) {
      ...
      ourHwnd = renderOutput->GetWindow()->GetWindowHandle();

      ImGuiIO & io = ::ImGui::GetIO();
      io.KeyMap[ ImGuiKey_Tab ] = VK_TAB;  // Keyboard mapping. ImGui will use those indices to peek into the
      ...
      // Load the imgui-shader state
      {
        ...
        ASSERT( ourProgramPipeline.IsValid() );
      }
      ...
      // Depth-stencil state (no depth testing)
      {
        ...
        ASSERT( ourDepthStencilState.IsValid() );
      }
      ...
      for ( uint iCmdList = 0u; iCmdList < static_cast< uint >( _draw_data->CmdListsCount ); ++iCmdList ) {
        ...
        for ( int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.size(); cmd_i++ ) {
          ...
          if ( pcmd->UserCallback ) {
            ...
            const GpuResourceView * textureView = RenderCore::GetTextureView( ourFontTexture );
    ...
    void Shutdown() {
      ourRenderOutput = RenderOutputHandle{};

      if ( ourFontTexture.IsValid() )
        RenderCore::DeleteTextureView( ourFontTexture );
      ourFontTexture = TextureViewHandle{};
      if ( ourFontTextureTex.IsValid() )
        RenderCore::DeleteTexture( ourFontTextureTex );
      ourFontTextureTex = TextureHandle{};

      ourProgramPipeline = ShaderPipelineHandle{};
      ourBlendState = BlendStateHandle{};
      ourDepthStencilState = DepthStencilStateHandle{};
