...
namespace Fancy {
  ...
  struct Profiler {
    ...
    struct ScopedGpuMarker {
      ScopedGpuMarker( CommandList * aCommandList, const char * aName, uint16 aTag );
      ~ScopedGpuMarker();

      CommandList * myCtx = nullptr;
    };
    ...
    static const CircularArray< FrameData > & GetRecordedFrames( Timeline aTimeline ) {
      return ourRecordedFrames[ aTimeline ];
    }
    static const CircularArray< SampleNode > & GetRecordedSamples( Timeline aTimeline ) {
      return ourRecordedSamples[ aTimeline ];
    }
