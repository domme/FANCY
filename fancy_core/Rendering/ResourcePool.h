...
namespace Fancy {
  ...
  template < typename T, uint Cap = 256, typename DescType = NoCacheType > class ResourcePool {
  public:
    ...
    Handle Add( T * aResource, uint64 aHash ) {
      static_assert( !std::is_same< DescType, NoCacheType >::value, "Add(resource, hash) requires a cached pool with DescType != NoCacheType" );
    ...
    Handle Add( T * aResource, const DescType & aDesc ) {
      static_assert( !std::is_same< DescType, NoCacheType >::value, "Add(resource, desc) requires a cached pool with DescType != NoCacheType" );
      return Add( aResource, DescType::Hash( aDesc ) );
    }
    //---------------------------------------------------------------------------//
    // Add without cache (non-cached pools only)
    Handle Add( T * aResource ) {
      static_assert( std::is_same< DescType, NoCacheType >::value, "Add(resource) requires a non-cached pool with DescType == NoCacheType" );
      ASSERT( aResource != nullptr );
      uint index;
      if ( !myFreeIndices.empty() ) {
        index = myFreeIndices.back();
    ...
    const CacheMap & GetCache() const {
      static_assert( !std::is_same< DescType, NoCacheType >::value, "GetCache() requires a cached pool with DescType != NoCacheType" );
      return myCache;
    }
    //---------------------------------------------------------------------------//
