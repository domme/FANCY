...
namespace Fancy {
  ...
  public:
    ...
    RtShaderBindingTableRange GetRayGenRange() const {
      ASSERT( myProperties.myNumRaygenShaderRecords > 0 );
      return GetRange( RT_SHADER_IDENTIFIER_TYPE_RAYGEN );
    }
    RtShaderBindingTableRange GetMissRange() const {
      ASSERT( myProperties.myNumMissShaderRecords > 0 );
      return GetRange( RT_SHADER_IDENTIFIER_TYPE_MISS );
    }
    RtShaderBindingTableRange GetHitRange() const {
      ASSERT( myProperties.myNumHitShaderRecords > 0 );
      return GetRange( RT_SHADER_IDENTIFIER_TYPE_HIT );
    }
