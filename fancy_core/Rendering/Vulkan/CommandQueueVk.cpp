...
//---------------------------------------------------------------------------//
  void CommandQueueVk::ResolveResourceHazardData(CommandList* aCommandList)
  {
    ...
    for (auto it : cmdListVk->myLocalHazardData)
    {
      ...
      GpuResourceHazardDataVk & globalHazardData = const_cast< GpuResource * >( resource )->GetVkData()->myHazardData;
      ...
      for (uint subIdx = 0u; subIdx < (uint) localHazardData.mySubresources.size(); ++subIdx)
      {
        ...
        if (resource->IsTexture())
        {
          ...
          if (!couldMerge)
          {
            ...
            barrier.myImage = resource->GetVkData()->myImage;
        ...
        else
        {
          ...
          barrier.myBuffer = resource->GetVkData()->myBufferData.myBuffer;
