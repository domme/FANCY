...
//---------------------------------------------------------------------------//
    ...
    uint someFlags, const char* aName, CommandList* cmdList)
  {
    ...
    myBuffer = RenderCore::CreateBuffer( bufferProps,
                                         StaticString<128>( "%s_%s_buffer", aName ? aName : "Unnamed",
                                                            isBLAS ? "BLAS" : "TLAS" ) );
    ASSERT( myBuffer.IsValid() );
    GpuBuffer * myBufferPtr = RenderCore::GetBuffer( myBuffer );
    ASSERT( myBufferPtr != nullptr );
    
    VkAccelerationStructureCreateInfoKHR asCreateInfo{ VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR };
    asCreateInfo.type = VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR;
    asCreateInfo.size = asSizeInfo.accelerationStructureSize;
    asCreateInfo.createFlags = 0;
    asCreateInfo.buffer = myBufferPtr->GetVkData()->myBufferData.myBuffer;
    ...
    GpuBufferHandle buildTempBuffer = RenderCore::CreateBuffer(
        tempBufferProps, StaticString<128>( "%s_%s_tempBuffer", aName ? aName : "Unnamed", isBLAS ? "BLAS" : "TLAS" ) );
    ASSERT( buildTempBuffer.IsValid() );
    GpuBuffer * buildTempBufferPtr = RenderCore::GetBuffer( buildTempBuffer );
    ASSERT( buildTempBufferPtr != nullptr );

    asBuildInfo.dstAccelerationStructure = myAccelerationStructure;
    asBuildInfo.scratchData.deviceAddress = buildTempBufferPtr->GetDeviceAddress();
    ...
    myBufferPtr->GetVkData()->myBufferData.myAccelerationStructureAddress =
        VkExt::vkGetAccelerationStructureDeviceAddressKHR( device, &accelerationDeviceAddressInfo );
    myBufferPtr->GetVkData()->myBufferData.myAccelerationStructure = myAccelerationStructure;
    ...
    if (!isBLAS)
    {
      ...
      myTopLevelBufferRead =
          RenderCore::CreateBufferView( myBufferPtr, viewProps, StaticString<128>( "%s_TLAS_bufferView", aName ? aName : "Unnamed" ) );
      ASSERT( myTopLevelBufferRead.IsValid() );
    }

    RenderCore::DeleteBuffer( buildTempBuffer );
  }
//---------------------------------------------------------------------------//
...
//---------------------------------------------------------------------------//
  void RtAccelerationStructureVk::Destroy()
  {
    ...
    myBuffer = GpuBufferHandle{};
    myTopLevelBufferRead = GpuBufferViewHandle{};
  }
//---------------------------------------------------------------------------//
}
