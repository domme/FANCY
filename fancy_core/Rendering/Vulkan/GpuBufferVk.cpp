...
//---------------------------------------------------------------------------//
  ...
  {
    return myVkData.myBufferData.myBuffer != nullptr ? &myVkData : nullptr;
  }
//---------------------------------------------------------------------------//
...
//---------------------------------------------------------------------------//
//---------------------------------------------------------------------------//
  GpuBufferViewVk::GpuBufferViewVk( GpuBuffer * aBuffer, const GpuBufferViewProperties & someProperties,
                                    const char * aName )
    : GpuBufferView(aBuffer, someProperties, aName)
  {
    ...
    if (myProperties.myFormat != DataFormat::UNKNOWN && !myProperties.myIsStructured && !myProperties.myIsRaw)
      nativeData.myView.myBuffer = CreateVkBufferView(aBuffer, someProperties);
    ...
    if (!someProperties.myIsConstantBuffer)
    {
      ...
      GpuBufferVk* bufferVk = static_cast<GpuBufferVk*>(aBuffer);
      ...
      if (someProperties.myIsRtAccelerationStructure)
      {
        ...
        nativeData.myGlobalDescriptor = RenderCore::GetPlatformVk()->AllocateAndWriteGlobalRTASDescriptor(accelerationStructure, name.c_str());
      ...
      else
      {
        ...
        nativeData.myGlobalDescriptor = RenderCore::GetPlatformVk()->AllocateAndWriteGlobalResourceDescriptor(globalType, descriptorInfo, name.c_str());
        myGlobalDescriptorIndex = nativeData.myGlobalDescriptor.myIndex;
      }
    }
...
//---------------------------------------------------------------------------//
  GpuBufferViewVk::~GpuBufferViewVk()
  {
    ...
    const GpuResourceViewDataVk & nativeData = myVkData;
...
//---------------------------------------------------------------------------//
  ...
  {
    return myVkData.myView.myBuffer;
  }
//---------------------------------------------------------------------------//
}
