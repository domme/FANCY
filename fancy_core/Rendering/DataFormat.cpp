...
#define DECLARE_DATA_FORMAT( name, byteSize, byteSizePlane0, byteSizePlane1, numComponents, numPlanes, depthStencil,   \
                             srgb, compressed, isUintInt, blockSize )                                                  \
  { DataFormat::name, byteSize, byteSizePlane0, byteSizePlane1, numComponents, numPlanes,                              \
    depthStencil,     srgb,     compressed,     isUintInt,      blockSize },
#include "DataFormatList.h"
#undef DECLARE_DATA_FORMAT
    };

    STATIC_ASSERT( ARRAY_LENGTH( locOurDataFormats ) == DataFormat::NUM,
                   "DataFormatInfos don't match with the number of DataFormats" );
