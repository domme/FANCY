...
namespace Fancy {
  //---------------------------------------------------------------------------//
  GpuBuffer::GpuBuffer() : GpuResource( GpuResourceType::BUFFER ), myAlignment( 0u ) {}
  //---------------------------------------------------------------------------//
  void * GpuBuffer::Map( GpuResourceMapMode aMapMode, uint64 anOffset, uint64 aSize ) const {
    ...
    // TODO: Ideally this should only wait for the last fence per queue this resource written last. But we don't have
    // this data (yet?)
