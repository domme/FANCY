...
DXGI_FORMAT RenderCore_PlatformDX12::ResolveFormat( DataFormat aFormat ) {
  switch ( aFormat ) {
    ...
    case DataFormat::BGRA_8:
      return DXGI_FORMAT_B8G8R8A8_UNORM;
    case DataFormat::RG_8:
      return DXGI_FORMAT_R8G8_UNORM;
    case DataFormat::R_8:
      return DXGI_FORMAT_R8_UNORM;
    case DataFormat::RGBA_16:
      return DXGI_FORMAT_R16G16B16A16_UNORM;
    case DataFormat::RG_16:
      return DXGI_FORMAT_R16G16_UNORM;
    case DataFormat::R_16:
    ...
    default:
      ASSERT( false, "Missing implementation or unsupported format" );
      return DXGI_FORMAT_R8G8B8A8_UNORM;
  }
}
//---------------------------------------------------------------------------//
DataFormat RenderCore_PlatformDX12::ResolveDXGIFormat( DXGI_FORMAT aFormat ) {
  switch ( aFormat ) {
    case DXGI_FORMAT_UNKNOWN:
      return DataFormat::UNKNOWN;
    // case DXGI_FORMAT_R32G32B32A32_TYPELESS:
    case DXGI_FORMAT_R32G32B32A32_FLOAT:
      return DataFormat::RGBA_32F;
    ...
    // case DXGI_FORMAT_D32_FLOAT: break;
    ...
    //  case DXGI_FORMAT_R9G9B9E5_SHAREDEXP: break;
    ...
    // case DXGI_FORMAT_BC2_UNORM: break;
    // case DXGI_FORMAT_BC2_UNORM_SRGB: break;
    ...
    case DXGI_FORMAT_BC7_UNORM_SRGB:
      return BC7_SRGB;
    // case DXGI_FORMAT_AYUV: break;
    // case DXGI_FORMAT_Y410: break;
    // case DXGI_FORMAT_Y416: break;
    // case DXGI_FORMAT_NV12: break;
    // case DXGI_FORMAT_P010: break;
    // case DXGI_FORMAT_P016: break;
    // case DXGI_FORMAT_420_OPAQUE: break;
    // case DXGI_FORMAT_YUY2: break;
    // case DXGI_FORMAT_Y210: break;
    // case DXGI_FORMAT_Y216: break;
    // case DXGI_FORMAT_NV11: break;
    // case DXGI_FORMAT_AI44: break;
    // case DXGI_FORMAT_IA44: break;
    // case DXGI_FORMAT_P8: break;
    // case DXGI_FORMAT_A8P8: break;
    // case DXGI_FORMAT_B4G4R4A4_UNORM: break;
    // case DXGI_FORMAT_P208: break;
    // case DXGI_FORMAT_V208: break;
    // case DXGI_FORMAT_V408: break;
    // case DXGI_FORMAT_SAMPLER_FEEDBACK_MIN_MIP_OPAQUE: break;
    // case DXGI_FORMAT_SAMPLER_FEEDBACK_MIP_REGION_USED_OPAQUE: break;
    // case DXGI_FORMAT_FORCE_UINT: break;
    default:
      ASSERT( false, "Missing implementation" );
      return DataFormat::UNKNOWN;
  }
...
DXGI_FORMAT RenderCore_PlatformDX12::GetTypelessFormat( DXGI_FORMAT aFormat ) {
  switch ( aFormat ) {
    ...
    case DXGI_FORMAT_R10G10B10A2_TYPELESS:
      // case DXGI_FORMAT_R11G11B10_FLOAT  // This is most likely not a valid format to cast into from
...
D3D12_DESCRIPTOR_HEAP_TYPE RenderCore_PlatformDX12::GetDescriptorHeapType( const GpuResourceViewType & aViewType ) {
  switch ( aViewType ) {
    ...
    case GpuResourceViewType::UAV:
      return D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    case GpuResourceViewType::DSV:
      return D3D12_DESCRIPTOR_HEAP_TYPE_DSV;
    case GpuResourceViewType::RTV:
...
D3D12_DESCRIPTOR_RANGE_TYPE RenderCore_PlatformDX12::GetDescriptorRangeType( const GpuResourceViewType & aViewType ) {
  switch ( aViewType ) {
    ...
    default:
      ASSERT( false, "Missing implementation or invalid view type for descriptor range type" );
      return D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
  }
}
//---------------------------------------------------------------------------//
D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE
RenderCore_PlatformDX12::GetRtAccelerationStructureType( RtAccelerationStructureType aType ) {
  switch ( aType ) {
    case RtAccelerationStructureType::BOTTOM_LEVEL:
      return D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;
    case RtAccelerationStructureType::TOP_LEVEL:
...
RenderCore_PlatformDX12::GetRaytracingBVHGeometryType( RtAccelerationStructureGeometryType aGeoType ) {
  switch ( aGeoType ) {
    case RtAccelerationStructureGeometryType::TRIANGLES:
      return D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES;
    case RtAccelerationStructureGeometryType::AABBS:
      return D3D12_RAYTRACING_GEOMETRY_TYPE_PROCEDURAL_PRIMITIVE_AABBS;
    default:
      ASSERT( false, "Not implemented" ) return D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES;
  }
}
//---------------------------------------------------------------------------//
...
RenderCore_PlatformDX12::RenderCore_PlatformDX12( const RenderPlatformProperties & someProperties )
    : RenderCore_Platform( RenderPlatformType::DX12, someProperties ), myGpuTicksToMsFactor{} {
...
bool RenderCore_PlatformDX12::InitInternalResources() {
  ...
  myGpuMemoryAllocators[ (uint) GpuMemoryType::BUFFER ][ (uint) CpuMemoryAccessType::CPU_WRITE ].reset(
      new GpuMemoryAllocatorDX12( GpuMemoryType::BUFFER, CpuMemoryAccessType::CPU_WRITE, 64 * SIZE_MB ) );
  myGpuMemoryAllocators[ (uint) GpuMemoryType::BUFFER ][ (uint) CpuMemoryAccessType::CPU_READ ].reset(
      new GpuMemoryAllocatorDX12( GpuMemoryType::BUFFER, CpuMemoryAccessType::CPU_READ, 64 * SIZE_MB ) );

  myGpuMemoryAllocators[ (uint) GpuMemoryType::TEXTURE ][ (uint) CpuMemoryAccessType::NO_CPU_ACCESS ].reset(
      new GpuMemoryAllocatorDX12( GpuMemoryType::TEXTURE, CpuMemoryAccessType::NO_CPU_ACCESS, 64 * SIZE_MB ) );
  myGpuMemoryAllocators[ (uint) GpuMemoryType::TEXTURE ][ (uint) CpuMemoryAccessType::CPU_WRITE ].reset(
      new GpuMemoryAllocatorDX12( GpuMemoryType::TEXTURE, CpuMemoryAccessType::CPU_WRITE, 16 * SIZE_MB ) );
  myGpuMemoryAllocators[ (uint) GpuMemoryType::TEXTURE ][ (uint) CpuMemoryAccessType::CPU_READ ].reset(
      new GpuMemoryAllocatorDX12( GpuMemoryType::TEXTURE, CpuMemoryAccessType::CPU_READ, 16 * SIZE_MB ) );

  myGpuMemoryAllocators[ (uint) GpuMemoryType::RENDERTARGET ][ (uint) CpuMemoryAccessType::NO_CPU_ACCESS ].reset(
      new GpuMemoryAllocatorDX12( GpuMemoryType::RENDERTARGET, CpuMemoryAccessType::NO_CPU_ACCESS, 64 * SIZE_MB ) );
  myGpuMemoryAllocators[ (uint) GpuMemoryType::RENDERTARGET ][ (uint) CpuMemoryAccessType::CPU_WRITE ].reset(
      new GpuMemoryAllocatorDX12( GpuMemoryType::RENDERTARGET, CpuMemoryAccessType::CPU_WRITE, 16 * SIZE_MB ) );
  myGpuMemoryAllocators[ (uint) GpuMemoryType::RENDERTARGET ][ (uint) CpuMemoryAccessType::CPU_READ ].reset(
      new GpuMemoryAllocatorDX12( GpuMemoryType::RENDERTARGET, CpuMemoryAccessType::CPU_READ, 16 * SIZE_MB ) );
...
void RenderCore_PlatformDX12::InitNullDescriptors() {
  // Create null descriptors to use as binding-dummies in descriptor tables where some elements are optimized away (e.g.
  // because of unused resources)
...
//---------------------------------------------------------------------------//
TextureView * RenderCore_PlatformDX12::CreateTextureView( const SharedPtr< Texture > & aTexture,
                                                          const TextureViewProperties & someProperties,
                                                          const char * aDebugName /* = nullptr */ ) {
  return new TextureViewDX12( aTexture, someProperties, aDebugName );
}
//---------------------------------------------------------------------------//
GpuBufferView * RenderCore_PlatformDX12::CreateBufferView( const SharedPtr< GpuBuffer > & aBuffer,
...
const DescriptorDX12 & RenderCore_PlatformDX12::GetNullDescriptor( D3D12_DESCRIPTOR_RANGE_TYPE aType,
  ...
  switch ( aType ) {
    ...
    case D3D12_DESCRIPTOR_RANGE_TYPE_UAV:
      return myUAVNullDescriptors[ (uint) aResouceDimension ];
    case D3D12_DESCRIPTOR_RANGE_TYPE_CBV:
      return myCBVNullDescriptor;
    case D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER:
      return mySamplerNullDescriptor;
    default:
      ASSERT( false );
      return mySamplerNullDescriptor;
  }
