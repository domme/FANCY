...
namespace Fancy {
  ...
  //---------------------------------------------------------------------------//
  void TextureDX12::Create( const TextureProperties & someProperties, const char * aName /* = nullptr */,
                            const TextureSubData * someInitialDatas /* = nullptr */, uint aNumInitialDatas /*= 0u*/ ) {
    Destroy();
    ...
    const uint minSide = ( myProperties.myDimension == GpuResourceDimension::TEXTURE_3D )
                             ? glm::min( myProperties.myWidth, myProperties.myHeight, myProperties.myDepthOrArraySize )
                             : glm::min( myProperties.myWidth, myProperties.myHeight );
    ...
    *hazardData = GpuResourceHazardDataDX12();
    ...
    // expose a preference as a flag to the caller?)
    /*
    ...
    CheckD3Dcall(device->CreateCommittedResource(&heapDesc.Properties, heapDesc.Flags, &resourceDesc, initialState,
    useOptimizeClearValue ? &clearValue : nullptr, IID_PPV_ARGS(&storageDx12->myResource)));
    */
  ...
  bool TextureViewDX12::CreateSRV( const Texture * aTexture, const TextureViewProperties & someProperties,
    ...
    if ( formatInfo.myIsDepthStencil ) {
      ...
      dxgiFormat = subresourceRange.myFirstPlane == 0 ? RenderCore_PlatformDX12::GetDepthViewFormat( dxgiFormat )
                                                      : RenderCore_PlatformDX12::GetStencilViewFormat( dxgiFormat );
    }
