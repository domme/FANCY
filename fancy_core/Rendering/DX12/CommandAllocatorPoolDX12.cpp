...
namespace Fancy {
  //---------------------------------------------------------------------------//
  CommandAllocatorPoolDX12::CommandAllocatorPoolDX12( CommandListType aType ) : myCommandListType( aType ) {}
  //---------------------------------------------------------------------------//
  CommandAllocatorPoolDX12::~CommandAllocatorPoolDX12() {
    UpdateAvailableAllocators();
    ASSERT( myAvailableAllocators.size() == myAllocatorPool.size(), "There are still some command allocators in flight when destroying the allocator pool" );
  }
  //---------------------------------------------------------------------------//
  ...
  ID3D12CommandAllocator * CommandAllocatorPoolDX12::GetNewAllocator() {
    ...
    ASSERT_HRESULT( RenderCore::GetPlatformDX12()->GetDevice()->CreateCommandAllocator( nativeCmdListType, IID_PPV_ARGS( &allocator ) ) );
