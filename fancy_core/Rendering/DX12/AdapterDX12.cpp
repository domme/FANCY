...
namespace Fancy {
  ...
  D3D12_FILL_MODE Adapter::toNativeType( const FillMode & generalType ) {
    switch ( generalType ) {
      ...
      default:
        ASSERT( false, "Missing native values" );
        return D3D12_FILL_MODE_SOLID;
    }
  }
  //---------------------------------------------------------------------------//
  ...
  CpuMemoryAccessType Adapter::ResolveGpuMemoryAccessType( D3D12_HEAP_TYPE aHeapType ) {
    switch ( aHeapType ) {
      case D3D12_HEAP_TYPE_DEFAULT:
        return CpuMemoryAccessType::NO_CPU_ACCESS;
      case D3D12_HEAP_TYPE_UPLOAD:
        return CpuMemoryAccessType::CPU_WRITE;
      case D3D12_HEAP_TYPE_READBACK:
        return CpuMemoryAccessType::CPU_READ;
      default:
        ASSERT( false, "Missing implementation" );
        return CpuMemoryAccessType::NO_CPU_ACCESS;
    }
  ...
  D3D12_BLEND Adapter::toNativeType( const BlendFactor & generalType ) {
    switch ( generalType ) {
      ...
      case BlendFactor::SRC_ALPHA_CLAMPED:
      ...
      case BlendFactor::INV_CONSTANT_COLOR:
