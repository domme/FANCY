...
namespace Fancy {
  ...
  struct SubresourceRange {
    ...
    SubresourceLocation First() const {
  ...
  struct SubresourceIterator {
    ...
    const SubresourceLocation & operator*() const {
  ...
  struct TextureRegion {
    TextureRegion() : myPos( glm::uvec3( 0 ) ), mySize( glm::uvec3( UINT_MAX ) ) {}

    TextureRegion( glm::uvec3 aPos, glm::uvec3 aSize ) : myPos( std::move( aPos ) ), mySize( std::move( aSize ) ) {}
  ...
  struct TextureSubData {
    TextureSubData() : myData( nullptr ), myRowSizeBytes( 0u ), mySliceSizeBytes( 0u ), myTotalSizeBytes( 0u ) {}
