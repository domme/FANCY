...
#if FANCY_ENABLE_VK
  return GetPlatformType() == RenderPlatformType::VULKAN
             ? static_cast< RenderCore_PlatformVk * >( ourPlatformImpl.get() )
             : nullptr;
#else
  return nullptr;
#endif
...
void RenderCore::Init_0_Platform( const RenderPlatformProperties & someProperties ) {
  ...
  const bool useVulkan = commandLine->HasArgument( "vulkan" ) || commandLine->HasArgument( "vk" );
  ...
  switch ( platformType ) {
    ...
    case RenderPlatformType::VULKAN:
      ...
      break;
    default:
      ASSERT( false, "Unsupported render platform type" );
      break;
  }
