...
RtShaderBindingTable::RtShaderBindingTable( const RtShaderBindingTableProperties & someProps )
  ...
  ASSERT( mySbtBuffer.IsValid() );

  myMappedSbtData = ( uint8 * ) RenderCore::GetBuffer( mySbtBuffer )->Map( GpuResourceMapMode::WRITE_UNSYNCHRONIZED );
...
RtShaderBindingTableRange RtShaderBindingTable::GetRange( RtShaderIdentifierType aType ) const {
  ...
  return { RenderCore::GetBuffer( mySbtBuffer ), offsetBytes, sizeBytes, myAlignedShaderRecordSizeBytes };
}
