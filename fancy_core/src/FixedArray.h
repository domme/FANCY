...
//---------------------------------------------------------------------------//
  //template<class T, uint u32Capacity, template<class, uint> StorageType = FixedArrayDatastore_Stack>
  template<class T, uint u32Capacity>
  class FixedArray : FixedArrayDatastore_Stack<T, u32Capacity>
  {
    public:
      ...
      virtual ~FixedArray() {};
    //---------------------------------------------------------------------------//  
      uint capacity() const {return u32Capacity;}
      uint size() const {return m_u32Size;}
      bool empty() const {return m_u32Size == 0u;}
      bool IsFull() const { return m_u32Size == u32Capacity; }
      void clear() {m_u32Size = 0u;}
      void resize(uint uNewSize) {ASSERT(uNewSize <= u32Capacity, "Array too small"); m_u32Size = uNewSize; }
    ...
    //---------------------------------------------------------------------------//
      T& operator[](const uint u32Index) {ASSERT(u32Index < m_u32Size); return m_Array[u32Index];}
      const T& operator[](const uint u32Index) const {ASSERT(u32Index < m_u32Size); return m_Array[u32Index];}
    //---------------------------------------------------------------------------//
...
//---------------------------------------------------------------------------//
  //template<class T, uint capacity, class StorageType /* = FixedArrayDatastore_Stack */>
...
//---------------------------------------------------------------------------//
  ...
  void FixedArray<T, u32Capacity>::erase(const T& _anElement)
  {
    ...
    {
      for (uint i = theDeleteIndex + 1u; i < m_u32Size; ++i)
      {
