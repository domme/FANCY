...
GPUProgram::GPUProgram() : m_uShaderProgram( GLUINT_HANDLE_INVALID )
{
	//STATIC_ASSERT( ARRAY_LENGTH( sShaderSemantics ) == ShaderSemantics::NUM );
}

GPUProgram::GPUProgram( GPUProgram& other ) : m_uShaderProgram( GLUINT_HANDLE_INVALID )
{
	...
	GPUProgramResourceManager::GetInstance().AddResource( m_uShaderProgram );
}

GPUProgram::~GPUProgram()
{
	GPUProgramResourceManager::GetInstance().HandleDelete( m_uShaderProgram );

	deleteResources();
}

void GPUProgram::deleteResources()
{
...
void GPUProgram::LoadShader( const std::string& szVertexShaderPath, const std::string& szFragmentShaderPath )
{
	...
	GPUProgramResourceManager::GetInstance().AddResource( uProgram );
...
IUniform* GPUProgram::createAttributeFromDescription( const uint32& eType, const String& szName, int32 iGLlocation )
{
	IUniform* pUniform = NULL;
...
IUniform* GPUProgram::createUniformFromDescription( const uint32& eType, const String& szName, int32 iGLlocation )
{
	IUniform* pUniform = NULL;
...
IUniform* GPUProgram::GetAttributeByName( const String& szName )
{
...
IUniform* GPUProgram::GetUniformByName( const String& szName )
{
...
void GPUProgram::UniformChanged( IUniform* pUniform )
{
...
uint GPUProgram::GetVertexAttribLocation( const char* szVertexAttrname )
{
	return glGetAttribLocation( m_uShaderProgram, szVertexAttrname );
}

uint GPUProgram::GetUniformLocation( const char* szUniformLoc )
{
	return glGetUniformLocation( m_uShaderProgram, szUniformLoc );
}


void GPUProgram::ApplyShader() const
{
