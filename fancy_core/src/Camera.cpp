...
//---------------------------------------------------------------------------//
  void Camera::setViewInv(const glm::mat4& viewInv)
  {
    m_matViewInv = viewInv;
    onViewInvChanged();
  }
//---------------------------------------------------------------------------//
...
std::vector<glm::vec3> Camera::getWSfrustumCorners()
{
  ...
  float hFar = 2.0f * h2Far;
  float hNear = 2.0f * h2Near;
  float aspect = (float) Renderer::GetInstance().getScreenWidth() / (float) Renderer::GetInstance().getScreenHeight();
  float w2Far = ( hFar * aspect ) / 2.0f;
  float w2Near = ( hNear * aspect ) / 2.0f;
