...
namespace Fancy {
   ...
   * It will grow in pages of PageSize and won't re-use memory from removed elements and instead always appends new
   * elements at the end of the pages - possibly allocating a new page. Pages are only removed if all of its elements
   * have been removed. This is to make it more likely that linked elements are close to each other in memory,
   * increasing cache-efficiency. Because of that, the GrowingList is intended for semi-ordered adds and removes.
   * Completely randomized add/remove might make only one element used per page, increasing the memory-requirements to
   * PageSize * NumberOfListElements as a worst case-scenario.
   */
  template < class T, uint64 PageSize > class GrowingList {
  public:
    struct Element;
    ...
    struct Iterator {
      ...
      T & operator*() {
        ASSERT( myElement != nullptr );
        return myElement->myData;
      }
      T * operator->() {
        ASSERT( myElement != nullptr );
        return &myElement->myData;
      ...
      explicit operator bool() const {
        return myElement != nullptr;
      }
      bool operator==( const Iterator & anOther ) const {
        return myElement == anOther.myElement;
      }
      bool operator!=( const Iterator & anOther ) const {
        return myElement != anOther.myElement;
      }
      Iterator Next() const {
        return myElement != nullptr ? myElement->myNext : nullptr;
      }
      Iterator Prev() const {
        return myElement != nullptr ? myElement->myPrev : nullptr;
      }
      bool HasNext() const {
        return myElement && myElement->myNext;
      }
      bool HasPrev() const {
        return myElement && myElement->myPrev;
      }
      Iterator operator++();
      Iterator operator--();
  ...
  //---------------------------------------------------------------------------//
  template < class T, uint64 PageSize > GrowingList< T, PageSize >::GrowingList() {}
  //---------------------------------------------------------------------------//
  template < class T, uint64 PageSize > GrowingList< T, PageSize >::~GrowingList() {
    DestroyAll();
  }
  ...
  //---------------------------------------------------------------------------//
  template < class T, uint64 PageSize > T & GrowingList< T, PageSize >::Back() {
    ASSERT( !IsEmpty() );
  ...
  typename GrowingList< T, PageSize >::Iterator GrowingList< T, PageSize >::RemoveInternal( Iterator aPos,
    ...
    // if (page == myTailPage && page->myNextFreeElementIdx > 0 && element ==
    // &page->myElements[page->myNextFreeElementIdx - 1])
  ...
  //---------------------------------------------------------------------------//
  template < class T, uint64 PageSize > bool GrowingList< T, PageSize >::IsEmpty() const {
    return myHeadElement == nullptr;
  }
  //---------------------------------------------------------------------------//
  template < class T, uint64 PageSize > void GrowingList< T, PageSize >::DestroyAll() {
