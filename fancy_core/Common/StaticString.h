...
namespace Fancy {
  //---------------------------------------------------------------------------//
  template < uint BufferSize > struct StaticString {
    StaticString() : myBuffer{ 0u }, myNextFree( 0u ) {}

    explicit StaticString( const char * aFormatString, ... ) : myBuffer{ 0u }, myNextFree( 0u ) {
      va_list args;
    ...
    template < uint BufferSize2 > StaticString< BufferSize > & operator=( const StaticString< BufferSize2 > & aStr ) {
    ...
    template < uint BufferSize2 > bool operator==( const StaticString< BufferSize2 > & aStr ) const {
      uint len = aStr.Size();
      if ( len != Size() )
        return false;
    ...
    bool IsEmpty() const {
      return myNextFree == 0u;
    }
    bool IsFull() const {
      return myNextFree == BufferSize;
    }
  ...
  //---------------------------------------------------------------------------//
  template < uint PoolSize > struct CircularStringBufferSized {
