...
namespace Fancy {
  //---------------------------------------------------------------------------//
  PagedLinearAllocator::PagedLinearAllocator( uint64 aPageSize ) : myPageSize( aPageSize ) {}
  //---------------------------------------------------------------------------//
  ...
  void PagedLinearAllocator::Free( const Block & aBlockToFree ) {
    ...
    if ( page.myOpenAllocs == 0 )  // Was this the last allocation from the page -> remove the page completely
    {
      ...
      {
        ASSERT( firstBlockInPage.Next() == lastBlockInPage );  // There must be exactly two free blocks, so the last
                                                               // block must directly follow the first
      ...
      // Remove all free blocks that belong to this page. Remove them in reverse order to make it more likely for
      // myFreeList to re-use existing block-items instead of allocating new ones when adding list-items again
