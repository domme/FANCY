...
namespace Fancy {
  ...
  void InputState::OnWindowEvent( UINT message, WPARAM wParam, LPARAM lParam, bool * /*aWasHandled*/ ) {
    switch ( message ) {
      case WM_MBUTTONDOWN:
        myMouseBtnMask |= MOUSE_BTN_MIDDLE;
        break;
      case WM_LBUTTONDOWN:
        myMouseBtnMask |= MOUSE_BTN_LEFT;
        break;
      case WM_RBUTTONDOWN:
        myMouseBtnMask |= MOUSE_BTN_RIGHT;
        break;
      case WM_SYSKEYDOWN:
      case WM_KEYDOWN:
        HandleKeyUpDownEvent( wParam, true );
        break;
      ...
      case WM_SYSKEYUP:
      case WM_KEYUP:
        HandleKeyUpDownEvent( wParam, false );
        break;
  ...
  void InputState::HandleKeyUpDownEvent( WPARAM wParam, bool aDown ) {
    ...
    if ( wParam >= 0x30 && wParam <= 0x5A ) {
      ...
      switch ( wParam ) {
        case VK_SHIFT:
          myModifierKeyMask = aDown ? myModifierKeyMask | MOD_KEY_SHIFT : ( myModifierKeyMask & ~MOD_KEY_SHIFT );
          break;
        case VK_MENU:
          myModifierKeyMask = aDown ? myModifierKeyMask | MOD_KEY_ALT : ( myModifierKeyMask & ~MOD_KEY_ALT );
          break;
        case VK_CONTROL:
          myModifierKeyMask = aDown ? ( myModifierKeyMask | MOD_KEY_CTRL ) : ( myModifierKeyMask & ~MOD_KEY_CTRL );
          break;
      }
    }
  }
