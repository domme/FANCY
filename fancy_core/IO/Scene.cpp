...
namespace Fancy {
  //---------------------------------------------------------------------------//
  Scene::Scene( const SceneData & aData ) : myInstances( aData.myInstances ) {
    myMeshes.reserve( aData.myMeshes.size() );
    for ( const MeshData & meshData : aData.myMeshes )
      myMeshes.push_back( Assets::CreateMeshHandle( meshData ) );

    myMaterials.reserve( aData.myMaterials.size() );
    for ( const MaterialDesc & material : aData.myMaterials )
      myMaterials.push_back( Assets::CreateMaterialHandle( material ) );
