...
// A small "hack" to set the opengl format in the initialization list:
// create a static adater-class around the QGL-Format description
struct QGLFormatAdapter
{
  QGLFormatAdapter(int p_MajorVersion, int p_MinorVersion, QGLFormat::OpenGLContextProfile p_Profile)
  {
    format.setVersion(p_MajorVersion, p_MinorVersion);
    format.setProfile(p_Profile);
    format.setOption(QGL::DeprecatedFunctions);
  }
  QGLFormat format;
};

static QGLFormatAdapter __glFormat(3, 1, QGLFormat::CompatibilityProfile);

EngineQGLwidget::EngineQGLwidget(QWidget *parent)
  : QGLWidget(__glFormat.format, parent),
  ...
  m_pStatsItemModel( NULL )
{
  ...
  int majorGLVersion = __glFormat.format.majorVersion();
  majorGLVersion = this->format().majorVersion();
...
void EngineQGLwidget::initializeGL()
{
  ...
  int majorGLVersion = this->format().majorVersion();
