...
bool MAT_FSquad_GaussianBlur::Init()
{
	...
	m_pShader->LoadShader( "Shader\\FSQuad_Postpro_simple.vert", "Shader\\FSQuad_GaussianBlur.frag" ); 
	...
	assignUniformSemantic( m_pShader,  "tGauss" ,  ShaderSemantics::TEXTURE_1D,  1 );
	assignUniformSemantic( m_pShader,  "tOffsets" ,  ShaderSemantics::TEXTURE_1D,  2 );

	assignUniformSemantic( m_pShader,  "kernelSize" ,  ShaderSemantics::KERNEL_SIZE );

	assignUniformSemantic( m_pShader,  "v2ImageSize" ,  ShaderSemantics::SCREEN_SIZE );
	assignUniformSemantic( m_pShader,  "v2SamplingDir" ,  ShaderSemantics::SEPERATED_SAMPLING_DIR );
	return true;
}
