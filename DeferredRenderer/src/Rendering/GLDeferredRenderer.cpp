...
void GLDeferredRenderer::RenderScene( SceneManager* pSceneManager )
{
	...
	m_pGLrenderer->prepareFrameRendering( m_pEngine->GetCurrentCamera(), pSceneManager->getRootNode()->getGlobalTransformMAT() ); //Recalculate and Update per-frame ("global") Uniforms
	...
	if( m_pGLrenderer->GetUseBloom() )
	{
		...
		m_pPassGaussianBlur->BlurTextureIntoFBO( m_uBrightPassTex_02, m_uBloomFBO_01, uBLURTEX_WIDTH, uBLURTEX_HEIGHT, GL_RGB32F, GL_RGB, GL_FLOAT, m_pGLrenderer, 5 );
		m_pPassGaussianBlur->BlurTextureIntoFBO( m_uBloomTex_01, m_uBloomFBO_01, uBLURTEX_WIDTH, uBLURTEX_HEIGHT, GL_RGB32F, GL_RGB, GL_FLOAT, m_pGLrenderer, 5 );
		m_pPassGaussianBlur->BlurTextureIntoFBO( m_uBloomTex_01, m_uBloomFBO_01, uBLURTEX_WIDTH, uBLURTEX_HEIGHT, GL_RGB32F, GL_RGB, GL_FLOAT, m_pGLrenderer, 5 );
		m_pPassGaussianBlur->BlurTextureIntoFBO( m_uBloomTex_01, m_uBloomFBO_01, uBLURTEX_WIDTH, uBLURTEX_HEIGHT, GL_RGB32F, GL_RGB, GL_FLOAT, m_pGLrenderer, 5 );
